...
static Point2f ParseVector2(const string aString)
{
  Point2f result;
...
static Point3f ParseVector3(const string aString)
{
  Point3f result;
...
void Import_OBJ(const char * aFileName, Mesh * aMesh)
{
  ...
  vector<Point3f> verticesArray(vertices.begin(), vertices.end());
  vector<Point2f> texCoordsArray(texCoords.begin(), texCoords.end());
  vector<Point3f> normalsArray(normals.begin(), normals.end());
  ...
  for(list<OBJFace>::iterator i = faces.begin(); i != faces.end(); ++ i)
  {
    ...
    for(list<OBJFaceNode>::iterator n = f.mNodes.begin(); n != f.mNodes.end(); ++ n)
    {
      ...
      if(nodeCount >= 3)
      {
		  ...
		  aMesh->Faces[idx/3].x = nodes[0][0];
		  aMesh->Faces[idx/3].y = nodes[1][0];
		  aMesh->Faces[idx/3].z = nodes[2][0];
...
void Export_OBJ(const char * aFileName, Mesh * aMesh, Options &aOptions)
{
  ...
  if(exportTexCoords)
  {
    for(unsigned int i = 0; i < aMesh->mTexCoords.size(); ++ i)
      f << "vt " << aMesh->mTexCoords[i].x << " " << aMesh->mTexCoords[i].y << endl;
  }
  ...
  {
	  unsigned int idx = aMesh->mIndices[i*3+0] + 1;
    f << "f " << idx << "/";
    if(exportTexCoords)
      f << idx;
    f << "/";
...
void ImportMesh(const char * aFileName, Mesh * aMesh)
{
	...
	ml->createMesh("male");
	mv->setModel(ml->getCurrentModel());
	mv->startViewer();
	mv->run();
